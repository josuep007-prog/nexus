...
class PainelWorker(QMainWindow):
    ...
    def _montar_ui(self):
        ...
        # Cabeçalho: identidade "máquina/robô", bem diferente da web
        ...
        self.bolinha = QLabel("●")
        self.bolinha.setObjectName("statusPonto")
        self.rotulo_status = QLabel("PARADO")
        self.rotulo_status.setObjectName("statusRotulo")
        lin.addWidget(self.bolinha)
        lin.addWidget(self.rotulo_status)
        lin.addStretch()

        lin.addWidget(QLabel("Verificar a cada"))
        ...
        self.btn_iniciar = QPushButton("▶  Iniciar robô")
        ...
        # Contadores rápidos (fila / entrega / manual)
        self.linha_contadores = QHBoxLayout()
        self.card_fila = self._card_contador("Na fila de automação", "0")
        self.card_entrega = self._card_contador("Processadas (aguard. entrega)", "0")
        self.card_manual = self._card_contador("Em atendimento manual", "0")
        ...
        self.tabela_fila.setMaximumHeight(170)
    ...
    def _card_contador(self, rotulo, valor):
        card = QFrame()
    ...
    def _marcar_rodando(self, rodando):
        self.rotulo_status.setText("RODANDO" if rodando else "PARADO")
        self.bolinha.setProperty("estado", "on" if rodando else "off")
        # força reaplicar o QSS (a cor da bolinha muda pelo 'estado')
        self.bolinha.style().unpolish(self.bolinha)
        self.bolinha.style().polish(self.bolinha)
