...
acesso (funcionário ou cliente) manualmente. Não há autocadastro no sistema —
este script é a única porta de entrada para criar usuários nesta versão.
...
def main():
    ...
    tipo_conta = ""
    while tipo_conta not in (TIPO_FUNCIONARIO, TIPO_CLIENTE):
        tipo_conta = input(f"Tipo de conta ({TIPO_FUNCIONARIO}/{TIPO_CLIENTE}): ").strip().lower()
        if tipo_conta not in (TIPO_FUNCIONARIO, TIPO_CLIENTE):
            print(f"  Valor inválido. Digite '{TIPO_FUNCIONARIO}' ou '{TIPO_CLIENTE}'.")
